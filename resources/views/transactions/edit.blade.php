    ...
    <div class="container mx-auto mt-5 mb-5">
        <form action="{{ route('transactions.update', $transaction->transaction_id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="registration_id" name="registration_id" class="form-control" value="{{ $transaction->registration_id }}" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" id="amount" name="amount" class="form-control" value="{{ $transaction->amount }}" required>
            </div>
            <div class="mb-4">
                <label for="payment_method" class="block text-gray-700 dark:text-gray-300">Metoda płatności</label>
                <select id="payment_method" name="payment_method" class="form-control" required>
                    @foreach(App\Enums\PaymentMethod::cases() as $method)
                        <option value="{{ $method->value }}" @if($transaction->payment_method->value == $method->value) selected @endif>{{ ucfirst(str_replace('_', ' ', $method->name)) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="status" class="block text-gray-700 dark:text-gray-300">Status</label>
                <input type="text" id="status" name="status" class="form-control" value="{{ $transaction->status }}" required>
            </div>
